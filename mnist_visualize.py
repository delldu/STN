...
if args.cuda:
    model.cuda()
image_dir = 'image/%s_angle%d_grid%d/' % (args.model, args.angle,
                                          args.grid_size)
...
paths = sorted(
    glob.glob('checkpoint/%s_angle%d_grid%d/*.pth' % (
        args.model,
        args.angle,
        args.grid_size,
    )))[::-1]
...
for pi, path in enumerate(paths):  # path index
    ...
    for si in range(batch_size):  # sample index
        ...
        source_image = Image.fromarray(source_array[si]).convert('RGB').resize(
            (128, 128))
        target_image = Image.fromarray(target_array[si]).convert('RGB').resize(
            (128, 128))
        ...
        for j in range(args.grid_size):
            for k in range(args.grid_size):
                ...
                if k > 0:  # connect to up
                    ...
                    draw.line((x1, y1, x2, y2), fill=(255, 0, 0))
        draw.text((10, 0),
                  'sample %03d, iter %03d' % (si, len(paths) - 1 - pi),
                  fill=(255, 0, 0),
                  font=font)
        canvas.save(image_dir + 'sample%03d_iter%03d.png' %
                    (si, len(paths) - 1 - pi))
