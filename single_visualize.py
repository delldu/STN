...
source_control_points = target_control_points + torch.Tensor(
    target_control_points.size()).uniform_(-0.1, 0.1)
...
source_coordinate = tps(torch.unsqueeze(source_control_points, 0))
grid = source_coordinate.view(1, target_height, target_width, 2)
canvas = torch.Tensor(1, 3, target_height, target_width).fill_(255)
target_image = grid_sample(source_image, grid, canvas)
target_image = target_image.data.numpy().squeeze().swapaxes(0,
                                                            1).swapaxes(1, 2)
